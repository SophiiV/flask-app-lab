...
def login():
    if request.method == "POST":
        ...
        if username == current_app.config["LOGIN_USER"] and password == current_app.config["LOGIN_PASSWORD"]:
            session["user"] = username
            flash("Вхід успішний. Ласкаво просимо!", "success")
            return redirect(url_for("users.profile"))
        flash("Невірний логін або пароль.", "error")
        return redirect(url_for("users.login"))
    return render_template("users/login.html", page_title="Вхід", content_title="Login")
